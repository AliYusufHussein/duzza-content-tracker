...
export default function ChannelsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Channels"
        subtitle="Every brand and every platform you publish to. Pre-loaded from your Content System."
                ...
                <div key={c.id} className="flex items-center justify-between rounded-md border border-border bg-secondary/30 px-2.5 py-1.5">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {c.link && <a href={c.link} target="_blank" rel="noreferrer" className="text-[11px] text-muted-foreground hover:text-foreground truncate flex items-center gap-1">
                      <ExternalLink className="h-3 w-3 shrink-0" /> <span className="truncate">{c.link.replace(/^https?:\/\//, '')}</span>
                    </a>}
                  </div>
                  <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0" onClick={() => remove(c.id)}><Trash2 className="h-3 w-3 text-muted-foreground" /></Button>
                </div>
              ))}
            </div>
          </Card>
